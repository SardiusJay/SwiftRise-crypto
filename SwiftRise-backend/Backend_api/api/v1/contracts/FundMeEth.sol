...
/**
    @title A contract for funding and withdrawing Ethereum from a contract.
    @author Eyang, Daniel Eyoh
    @notice Only owner is allowed to withdraw money from the contract.
    @dev This implements pricefeeds to generate ethereum current market value.
 */
contract FundMeEth {
    /* Type Declaration */
    using PriceConverter for uint256;
    enum FundMeEthState { // set the state of the contract.
        OPEN,
        PENDING
    }
    ...
    FundMeEthState private s_fundState;
    ...
    constructor(address priceFeedAddress) {
		...
		s_priceFeed = AggregatorV3Interface(priceFeedAddress); // The price feed i.e Current Etherium price in the market.
        s_fundState = FundMeEthState.OPEN; // set the initial state of the contract fundState
    }
    ...
    */
    function withdraw() public onlyOwner {
        // disable funding
        s_fundState = FundMeEthState.PENDING;
        ...
        if (success) {
            s_fundState = FundMeEthState.OPEN;
        } else {
            s_fundState = FundMeEthState.OPEN;
    ...
    function getUsdToEth(uint256 amountInUsd) public view returns (uint256) {
        return (amountInUsd.convertUsdToEth(s_priceFeed));
    }

    function getFundState() public view returns (FundMeEthState) {
        return s_fundState;
    }
